...
class Settings:
    ...
    azure_openai_text_deployment: str = os.getenv("AZURE_OPENAI_TEXT_DEPLOYMENT", "gpt-4.1-mini")
    azure_openai_api_version: str = os.getenv("AZURE_OPENAI_API_VERSION", "2024-08-01-preview")
    # When true, handwritten/image TRFs skip EasyOCR and go straight to vision.
    skip_ocr_for_vision: bool = os.getenv("SKIP_OCR_FOR_VISION", "true").lower() == "true"
    ...
    vision_max_completion_tokens: int = int(os.getenv("VISION_MAX_COMPLETION_TOKENS", "3000"))
    text_max_chars: int = int(os.getenv("TEXT_MAX_CHARS", "45000"))
    text_max_completion_tokens: int = int(os.getenv("TEXT_MAX_COMPLETION_TOKENS", "3500"))
    text_chunk_parallelism: int = int(os.getenv("TEXT_CHUNK_PARALLELISM", "3"))
    chat_max_completion_tokens: int = int(os.getenv("CHAT_MAX_COMPLETION_TOKENS", "250"))
    ...
    skip_vlm_min_ocr_confidence: float = float(os.getenv("SKIP_VLM_MIN_OCR_CONFIDENCE", "0.72"))
    ...
    guardrail_min_rerank_score: float = float(os.getenv("GUARDRAIL_MIN_RERANK_SCORE", "0.25"))
    guardrail_min_chunks: int = int(os.getenv("GUARDRAIL_MIN_CHUNKS", "1"))
    guardrail_coverage_threshold: float = float(os.getenv("GUARDRAIL_COVERAGE_THRESHOLD", "0.6"))
